...
const userSchema = new mongoose.Schema({
  ...
  email: { type: String, lowercase: true, required: true, sparse: true },
...
app.get("/guestLecture", function (req, res) {
...
generateSamniveshaId = async () => {
  ...
  while (true) {
    ...
    const user = await User.findOne({ samniveshaId: samniveshaId });
    if (!user) {
      return samniveshaId;
    }
  }
  ...
  if (!email) {
    ...
    newUser.save(function (err) {
      if (err) {
        ...
        res.render("success.ejs", { heading: content1, para: content2 });
      } else {
        var mailOptions = {
          from: process.env.EMAIL_ID,
          to: req.body.email,
          subject: "Samnivesha 2021",
          text:
            "Dear " +
            req.body.fullname +
            ",\n\n" +
            "Your Registration for Samnivesha'21 is successful.\n" +
